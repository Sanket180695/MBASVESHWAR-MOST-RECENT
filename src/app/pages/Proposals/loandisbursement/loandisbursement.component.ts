...
export class LoandisbursementComponent implements OnInit {
  ...
  save() {
    ...
    this.isButtonSpinning = true
    var nextStageId = 13
    if (this.data.LOAN_DISBURSEMENT_REMARK != undefined && this.data.LOAN_DISBURSEMENT_REMARK.trim() != '') {

      this.isButtonSpinning = true
      // //console.log(nextStageId, this.data.ID)
      this.api.updateLoanDisbursement(this.data.CURRENT_STAGE_ID, nextStageId, this.data.LOAN_DISBURSEMENT_REMARK, this.data.ID, 0, this.data.PROPOSAL_REPORT,this.data.MATURITY_DUE,this.data.MATURITY_DUE_DATE)
        .subscribe(successCode => {
          // //console.log(successCode)
          this.isButtonSpinning = false
